...
*   asm7080.sem    - From CHAT
*   asm7080.ptb    - From CHAT
*   asm7080.err    - From CHAT
*   asm7080.tok    - From CHAT
*
...
*   parsetable    - From CHAT, in asm7080.ptb
*   gototable     - From CHAT, in asm7080.ptb
...
tokval
Parser (char *expr, int *eindex, int rsize, int defr)
{
   uint16 *pt, *gp;
   tokval value, rvalue;
   int    i, j, index;
   ...
   token = Scanner (expr, &index, &value, toksym, &tterm, defer);
   if (token == 0) return(0);
   ...
   do
   {
         ...
         if ((tok == 127) || (tok == token))
	 {
            ...
            if (act == SHIFT_ACTION)
	    {
               ...
               token = Scanner (expr, &index, &value, toksym, &tterm,
				defer);
	       if (token == 0) return(0);
            }
