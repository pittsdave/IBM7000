2013/11/05 CTSS Source

In order to build and operate using the cross development utilities the code
has hacks. The hacked code is identified with "DGPHACK" in the right margin.
The DGPHACKed code only assembles when using asm7090 as it defines the symbol
XBUILD which conditionalizes the assembly. When assembly is done with native
FAP the XBUILD symbol is not defined and the original code will be assembled.


I. To Build:

1. Build the components needed, asm7090, lnk7090, s709, and utilities. The 
program versions used are the minimum for this release.

$ tar xzf asm7090-2.2.26.tar.gz
$ cd asm7090
$ make
$ make install

$ tar xzf lnk7090-2.2.2.tar.gz
$ cd lnk7090
$ make
$ make install
...
$ tar xzf ctss-<VERSION>.tar.gz
$ cd ctss

Edit the Makefile and change the TAPEDIR and S709DIR variables to point to the
directories of your choice. Then enter:

$ make

This will build the currently defined sources.
The tapes currently created are:
...
II. To initialize CTSS:

1. Build the components needed, the utilities and s709 (if not done previously).
...
2. Edit the command scripts, shown below, and change the TAPE and DASD
variables to point to your appropriate directories. The command scripts:
...
6. To install the software on the disks:

$ installctss
IBM 7094-CTSS Simulator 2.3.5

 ADDING UFD  M1416 CMFL01
...
7. Make a backup of your dasd images:
...
1. Start the simulator running the CTSS image:
...
There is no "standard" way of adding users to CTSS; so, I added the shell script
